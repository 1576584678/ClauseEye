...
| 本地存储 | **单文件库（node:sqlite 容器，主进程承载）＋ AES-256-GCM 逐条加密**；浏览器预览退回 IndexedDB | 单文件可备份 / 迁移、事务 + WAL 崩溃安全；不引入原生 SQLCipher（需三平台预编译），零编译依赖 |
...
> 承载方式：零后端纯前端（Vite + React + TS）+ Electron 桌面壳（自定义 `app://` 协议加载前端产物；密文落在主进程的单文件库 `<userData>/vault.sqlite`，浏览器预览退回 IndexedDB）。

### 10.1 MVP（v0.1.0）

- [x] 本地界面外壳 + 本地加密库（单文件 `vault.sqlite` + AES-256-GCM；浏览器退回 IndexedDB）
- [x] 导入 → 本地解析（PDF/DOCX/文本）→ 自动分类（关键词加权打分，11 类场景 + 兜底）
...
- [x] 规则库扩充：11 个场景包 + 通用包共 **186 条**规则，按「种子包 + v1 扩充包」两层组织，旧规则 id 保持稳定
...
- [x] SQLCipher / Tauri：落地为**单文件加密库**（主进程 node:sqlite 承载 AES-GCM 密文，零原生依赖，可整文件备份）；Tauri 评估后不迁移（IndexedDB / WASM OCR 需重写，收益不足），原生 SQLCipher 需三平台预编译扩展亦不引入

### 10.3 V1 收尾（v0.5.0）

- [x] 后台常驻：托盘常驻（默认开启，可在「提醒」页关闭）+ 开机自启（Windows / macOS 写入系统登录项，Linux 写 `~/.config/autostart/*.desktop`）；关闭窗口不再中断到期提醒，自启时以 `--hidden` 静默进托盘
- [x] 长扫描件 OCR：单次上限 20 → 120 页，每 20 页让出一次主线程并逐页释放渲染资源，进度按百分比刷新
- [x] 规则库继续扩充：新增二手房买卖 / 装修合同 / 驾培协议 3 个场景包（+36 条规则）与 11 条评测用例，合计 **186 条规则 / 26 条用例**

> 桌面壳偏好（托盘常驻 / 开机自启）存在 `<userData>/preferences.json`：窗口关闭时渲染进程可能尚未解锁保险箱，主进程必须能独立判断该不该退到托盘；该文件只含布尔开关，没有文档内容与密钥。
